...
<xsl:template match="manhead">.TH <xsl:value-of select="@prog-name" /><xsl:text> </xsl:text><xsl:value-of select="@section" /> &quot;<xsl:value-of select="@modification-date" />&quot; &quot;<xsl:value-of select="left-footer" />&quot; &quot;<xsl:value-of select="center-header" />&quot;</xsl:template>
