 ...
 */
#define DEFAULTUNARYOPDECL_( Ta ) virtual RefCountPtr< const Lang::Value > op( const RefCountPtr< const MetaPDF::Ta > & arg ) const;
#define DEFAULTUNARYOPDECL FORALLCLASSESM( DEFAULTUNARYOPDECL_ )

/* ... and an implementation.
 */
#define DEFAULTUNARYOPIMPL_( Ts, Ta ) RefCountPtr< const Lang::Value > MetaPDF::Ts::op( const RefCountPtr< const MetaPDF::Ta > & arg ) const { return this->throwNotApplicable( arg.getPtr( ) ); }
 ...
 * for each pair combination of classes.
 */
#define DEFAULTBINARYOPDECL__( Ta, Tb ) virtual RefCountPtr< const Lang::Value > op( const RefCountPtr< const MetaPDF::Ta > & arg1, const RefCountPtr< const MetaPDF::Tb > & arg2 ) const;
...
/* ... and an implementation.
 */
#define DEFAULTBINARYOPIMPL__( Ta, Tb ) RefCountPtr< const Lang::Value > Ast::BinaryInfixExpr::op( const RefCountPtr< const MetaPDF::Ta > & arg1, const RefCountPtr< const MetaPDF::Tb > & arg2 ) const { return throwNotApplicable( arg1.getPtr( ), arg2.getPtr( ) ); }
